...
mindmap
  root((razorpay_buildathon))
    ...
    submission["submission/ — what ships to Razorpay"]
      ...
      founder-email.md["founder-email.md (PARKED)"]
    outreach["outreach/ — real validation
    conversations only (Day 4)"]
    src["src/ — the system (from Day 6)"]
      agent["agent/ — thin: reason/plan"]
      tool["tool/ — thin: mocked Razorpay APIs"]
      safety["safety/ — DEEP: permissions/limits"]
      verification["verification/ — DEEP: the moat"]
      memory["memory/ — thin: state"]
      audit["audit/ — DEEP: action log"]
    eval["eval/ — adversarial harness (Day 11+)"]
